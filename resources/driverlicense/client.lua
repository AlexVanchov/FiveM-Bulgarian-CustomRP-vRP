...
function EndDTest()
        if Error > maxErrors then
			drawNotification("Скъсаха те с ".. Error.." ~r~грешки")
			onPtest = false
			EndTestTasks()
		else
			--local licID = 1
			...
			theorylock = false
			testlock = false
			drawNotification("Премина с ".. Error.." ~r~грешки")	
			EndTestTasks()
		end
end
...
Citizen.CreateThread(function()
    while true do
		...
		TriggerEvent("pNotify:SendNotification",{
            text = "Автомобила е <b style='color:#B22222'>повреден!</b><br /><br />Watch it!",
            type = "alert",
            timeout = (2000),
            layout = "bottomCenter",
            queue = "global"
		...
		elseif(IsControlJustReleased(1, 23)) and DamageControl == 1 then
			drawNotification("Не можете да оставите автомобила по време на изпита")
    	end
	...
	if onTestEvent == 1 then
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 255.13990783691,-1400.7319335938,29.5374584198, true) > 4.0001 then
		   DrawMarker(1,255.13990783691,-1400.7319335938,29.5374584198,0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 178, 236, 93, 155, 0, 0, 2, 0, 0, 0, 0)
			...
			N_0x80ead8e2e1d5d52e(onTestBlipp)
		    DrawMissionText2("Преминете към следващата точка!", 5000)
			onTestEvent = 2
		end
	end
	...
	if onTestEvent == 3 then
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),234.90780639648,-1345.3854980469, 29.542045593262, true) > 4.0001 then
		   DrawMarker(1,234.90780639648,-1345.3854980469, 29.542045593262,0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 178, 236, 93, 155, 0, 0, 2, 0, 0, 0, 0)
			...
			N_0x80ead8e2e1d5d52e(onTestBlipp)
			SetBlipRoute(onTestBlipp, 1)
		    DrawMissionText2("~r~Спри~s~ за да преминат ~y~пешеходците", 5000)
			...
			DrawMissionText2("~g~Чудесно!~s~ продължи движението!", 5000)
			onTestEvent = 4
		end
	end	
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),217.82102966309,-1410.5201416016,28.292112350464, true) > 4.0001 then
		   DrawMarker(1,217.82102966309,-1410.5201416016,28.292112350464,0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 178, 236, 93, 155, 0, 0, 2, 0, 0, 0, 0)
			...
			N_0x80ead8e2e1d5d52e(onTestBlipp)
			SetBlipRoute(onTestBlipp, 1)
		    DrawMissionText2("~r~Спри~s~ и се огледай в ~y~ЛЯВО~s~ преди да продължиш", 5000)
			PlaySound(-1, "RACE_PLACED", "HUD_AWARDS", 0, 0, 1)
			FreezeEntityPosition(GetVehiclePedIsUsing(GetPlayerPed(-1)), true) -- Freeze Entity
			Citizen.Wait(6000)
			FreezeEntityPosition(GetVehiclePedIsUsing(GetPlayerPed(-1)), false) -- Freeze Entity
			DrawMissionText2("~g~Чудесно!~s~ Продължете на ~y~ДЯСНО~s~!", 5000)
			drawNotification("Зона: ~y~Град - ~s~Ограничение: ~y~60 km/h\n~s~Грешки: ~y~".. Error.."/"..maxErrors)
			SpeedControl = 2
			onTestEvent = 5
			Citizen.Wait(4000)
		end
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),178.55052185059,-1401.7551269531,27.725154876709, true) > 4.0001 then
		   DrawMarker(1,178.55052185059,-1401.7551269531,27.725154876709,0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 178, 236, 93, 155, 0, 0, 2, 0, 0, 0, 0)
			...
			N_0x80ead8e2e1d5d52e(onTestBlipp)
			SetBlipRoute(onTestBlipp, 1)
		    DrawMissionText2("Следете сигналите на ~y~светофарите~s~ !", 5000)
			onTestEvent = 6
		end
	end	
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),-73.542953491211,-1364.3355712891,27.789325714111, true) > 4.0001 then
		   DrawMarker(1,-73.542953491211,-1364.3355712891,27.789325714111,0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 178, 236, 93, 155, 0, 0, 2, 0, 0, 0, 0)
			...
			N_0x80ead8e2e1d5d52e(onTestBlipp)
			SetBlipRoute(onTestBlipp, 1)
		    DrawMissionText2("Уверете се, че сте спрели за да преминат автомобилите!", 5000)
			onTestEvent = 8
		end
	end			
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),-453.79092407227,-1444.7265625,27.665870666504, true) > 4.0001 then
		   DrawMarker(1,-453.79092407227,-1444.7265625,27.665870666504,0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 178, 236, 93, 155, 0, 0, 2, 0, 0, 0, 0)
			...
			N_0x80ead8e2e1d5d52e(onTestBlipp)
			SetBlipRoute(onTestBlipp, 1)
		    DrawMissionText2("Време е да се качи на пътя, следи скоростта си и пътната обстановка!", 5000)
			PlaySound(-1, "RACE_PLACED", "HUD_AWARDS", 0, 0, 1)
			drawNotification("Зона: ~y~Магистрала - ~s~Ограничение: ~y~120 km/h\n~s~Грешки: ~y~".. Error.."/"..maxErrors)
			onTestEvent = 11
			SpeedControl = 3
			Citizen.Wait(4000)
		end
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),1225.7598876953,-1948.7922363281,38.718940734863, true) > 4.0001 then
		   DrawMarker(1,1225.7598876953,-1948.7922363281,38.718940734863,0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 178, 236, 93, 155, 0, 0, 2, 0, 0, 0, 0)
			...
			N_0x80ead8e2e1d5d52e(onTestBlipp)
			SetBlipRoute(onTestBlipp, 1)
			DrawMissionText2("Влизаш в град, следи скоростта си!", 5000)
			drawNotification("~r~Намали!\n~s~Зона: ~y~Град - ~s~ограничение: ~y~60 km/h\n~s~Грешки: ~y~".. Error.."/"..maxErrors)
			onTestEvent = 15
		end
	end		
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),1163.6030273438,-1841.7713623047,35.679168701172, true) > 4.0001 then
		   DrawMarker(1,1163.6030273438,-1841.7713623047,35.679168701172,0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 178, 236, 93, 155, 0, 0, 2, 0, 0, 0, 0)
			...
			SetBlipRoute(onTestBlipp, 1)
			PlaySound(-1, "RACE_PLACED", "HUD_AWARDS", 0, 0, 1)
		    DrawMissionText2("Добра работа, сега нека се върнем назад!", 5000)
			drawNotification("Зона: ~y~Град - ~s~Ограничение: ~y~60 km/h\n~s~Грешки: ~y~".. Error.."/"..maxErrors)
			SpeedControl = 2
			onTestEvent = 16
		end
...
RegisterNUICallback('close', function(data, cb)
  ...
  closeGui()
  cb('ok')
  DrawMissionText2("~b~Премина теста, сега можете да продължите с теста за шофиране", 2000)	
  theorylock = true
  testlock = false
  onTtest = false
end)
...
RegisterNUICallback('kick', function(data, cb)
  closeGui()
  cb('ok')
  DrawMissionText2("~r~Провалихте се на теста, може да опитате отново", 2000)	
  onTtest = false
  theorylock = false
end)
...
Citizen.CreateThread(function()
    while true do
		...
		DrawMissionText2("~r~Шофираш без книжка", 2000)			
		end
	end
end)
...
Citizen.CreateThread(function()
    while true do
		...
		TriggerEvent("pNotify:SendNotification",{
            text = "Караш бързо! <b style='color:#B22222'>Намали скоростта!</b><br /><br />Шофираш в зона с лимит на скоростта до <b style='color:#DAA520'>50 km/h</b>!",
            type = "alert",
            timeout = (2000),
            layout = "bottomCenter",
            queue = "global"
		...
		TriggerEvent("pNotify:SendNotification",{
            text = "Караш бързо! <b style='color:#B22222'>Намали скоростта!</b><br /><br />Шофираш в зона с лимит на скоростта до <b style='color:#DAA520'>60 km/h</b>!",
            type = "alert",
            timeout = (2000),
            layout = "bottomCenter",
            queue = "global"
		...
		TriggerEvent("pNotify:SendNotification",{
            text = "Караш бързо! <b style='color:#B22222'>Намали скоростта!</b><br /><br />Шофираш в зона с лимит на скоростта до <b style='color:#DAA520'>120 km/h</b>!",
            type = "alert",
            timeout = (2000),
            layout = "bottomCenter",
            queue = "global"
        ...
        if ( ped and inVehicle and DamageControl == 1 ) then
            if IsControlJustPressed(1, 73) then
                if (GetPedInVehicleSeat(vehicle, -1) == ped) then
                    if CruiseControl == 0 then
						...
						Citizen.Wait(1000)
				        DisplayHelpText("Нагласете максималната си скорост с ~INPUT_CELLPHONE_UP~ ~INPUT_CELLPHONE_DOWN~ контроли")
						PlaySound(-1, "COLLECTED", "HUD_AWARDS", 0, 0, 1)
                        CruiseControl = 1
                    else
                        SetEntityMaxSpeed(vehicle, Max)
						drawNotification("~y~Cruise Control: ~r~изключен")						
                        CruiseControl = 0
                    end
                else
				    drawNotification("Трябва да карате автомобил за да използвате тези клавиши")						
                end
            elseif IsControlJustPressed(1, 27) then
                if CruiseControl == 1 then
					...
					DisplayHelpText("Максималната скорост е ".. math.floor(speedLimit*3.6).. "kmh")
                end
	...
	while true do
		...
		for k,v in ipairs(dmvpedpos) do
			if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 1.0)then
				DisplayHelpText("Натисни ~INPUT_CONTEXT~ за да поговориш с ~y~инструктора")
				if(IsControlJustReleased(1, 38))then
						if talktodmvped then
						    Notify("~b~Добре дошли в ~h~Училището за курмуване!")
						    Citizen.Wait(500)
							DMVMenu()
							Menu.hidden = false
							talktodmvped = false
...
function DMVMenu()
	ClearMenu()
    options.menu_title = "Изпит за книжка"
	Menu.addButton("Вземи книжка","VehLicenseMenu",nil)
    Menu.addButton("Затвори","CloseMenu",nil) 
end

function VehLicenseMenu()
    ClearMenu()
    options.menu_title = "Лиценз"
	Menu.addButton("Започни теста 2000$","startptest",nil)
