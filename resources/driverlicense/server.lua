...
AddEventHandler("dmv:ttcharge", function()
	...
	if vRP.tryPayment({user_id,200}) then
		...
		vRPclient.notify(player,{"~r~Нямаш достатачно пари."})
	end
end)
...
local choice_asklc = {function(player,choice)
  vRPclient.getNearestPlayer(player,{10},function(nplayer)
    ...
    if nuser_id ~= nil then
      vRPclient.notify(player,{"Шофьорска книжка, моля..."})
      vRP.request({nplayer,"Искаш ли да си покажеш шофьорската книжка?",15,function(nplayer,ok)
        if ok then
          MySQL.query("vRP/dmv_search", {id = nuser_id}, function(rows, affected)
            if #rows > 0 then
			  vRPclient.notify(player,{"Шофьорска книжка: ~g~ИМА"})
			else
			  vRPclient.notify(player,{"Шофьорска книжка: ~r~НЯМА"})
            end
          end)
        else
...
end, "Провери шофьорската книжка на човека до теб."}
...
vRP.registerMenuBuilder({"police", function(add, data)
  ...
  if user_id ~= nil then
    ...
    if vRP.hasPermission({user_id,"police.askid"}) then
       choices["Провери за книжка"] = choice_asklc
    end
