		...
		if not InBlacklistedVehicle then
			...
			if nearPump and IsCloseToLastVehicle then
				...
				if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Излезте от автомобила, за да заредите автомобила.")
				elseif IsFueling then
					local position = GetEntityCoords(vehicle)

					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Натиснете ~g~G~w~ за да заредите вашият автомобил. ~r~R$" .. price .. "~w~.")
				...
				elseif fuel > 95.0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Вашият автомобил вече е зареден.")
				elseif cash <= 0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Нямате достатъчно пари за гориво.")
				else
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Натиснете ~g~G~w~ за да започнете да зареждате. ~r~R$2.00~w~/L.")
			...
			elseif NearVehicleWithJerryCan and not nearPump and Config.EnableJerryCans then
				...
				if IsFuelingWithJerryCan then
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Натиснете ~g~G~w~ за да спрете зареждането. Гориво: " .. fuel .. "%")
					...
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Натиснете ~g~G ~w~за да заредите гориво със галон.")
