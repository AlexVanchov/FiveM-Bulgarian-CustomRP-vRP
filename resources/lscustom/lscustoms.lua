...
local function DriveOutOfGarage(pos)
	Citizen.CreateThread(function()
		...
		if ok ~= null then
			...
			local ccolor = c1 .. "," .. c2
			local c1, c2 = GetVehicleExtraColours(veh)
			local cexcolor = c1 .. "," .. c2
			local neonr, neong, neonb = GetVehicleNeonLightsColour(veh)
			local neon = neonr .. "," .. neong .. "," .. neonb
			local neont = false
			...
			local tiresmr, tiresmg, tiresmb = GetVehicleTyreSmokeColor(veh)
			local tire = tiresmr .. "," .. tiresmg .. "," .. tiresmb
			local plate = GetVehicleNumberPlateTextIndex(veh)
			local wtint = GetVehicleWindowTint(veh)
			local whtype = GetVehicleWheelType(veh)
			local vtyreb = GetVehicleTyresCanBurst(veh)
			local vmod = ""
			for i = 0, 48 do
				if i == 18 or i == 22 or i == 20 then
					if IsToggleModOn(veh,i) then
						vmod = vmod .. tostring(i) .. "," .. GetVehicleMod(veh,i) .. ",true;"
					else
						vmod = vmod .. tostring(i) .. "," .. GetVehicleMod(veh,i) .. ",false;"
					end
				elseif i == 23 or i == 24 then
					if GetVehicleModVariation(veh, i) then
						vmod = vmod .. tostring(i) .. "," .. GetVehicleMod(veh,i) .. ",true;"
					else
						vmod = vmod .. tostring(i) .. "," .. GetVehicleMod(veh,i) .. ",false;"
					end
			...
			TriggerServerEvent("lscustom:sendDB",ups, name)
		end
						...
						if LSC_Config.oldenter then			
							...
							if GetDistanceBetweenCoords(outside.x,outside.y,outside.z,GetEntityCoords(ped)) <= f(5) then
								...
								if not tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
									...
									if pos.locked then
										...
										if not LSC_Config.lock then
											if IsControlJustPressed(1,201) then
												...
												drawTxt("Press ~b~ENTER~w~ to modify your ~g~car.",4,1,0.5,0.8,1.0,255,255,255,255)
											end
										else
											drawTxt("",4,1,0.5,0.8,1.0,255,255,255,255)
										end
									else
										if IsControlJustPressed(1,201) then
											inside = true
									...
									drawTxt("~r~Someone is inside , please wait.",4,1,0.5,0.8,1.0,255,255,255,255)
								end
							end
						else
								...
								if not tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
									...
									if pos.locked then
										...
										if not LSC_Config.lock then
											...
											drawTxt("~r~?????, ???? ??? ?????!",4,1,0.5,0.8,1.0,255,255,255,255)
										end
									else
										inside = true
										currentpos = pos
...
AddEventHandler("LSC:buttonSelected", function(name, button, canpurchase)
	...
	elseif mname == "wheel accessories" then
		...
		elseif button.name == "Pneus customizado" and  (button.purchased or CanPurchase(price, canpurchase)) then
			SetVehicleModKit(veh,0)
		...
		elseif button.name == "Pneu a prova de bala" and  (button.purchased or CanPurchase(price, canpurchase)) then
...
function CheckPurchases(m)
	...
	elseif name == "neon layout" then
		for i,b in pairs(m.buttons) do
			...
			elseif b.name == "Frontal,lateral e traseiro" then
	...
	elseif name == "wheel accessories" then
		for i,b in pairs(m.buttons) do
			if b.name == "Pneu de fábrica" then
				if GetVehicleModVariation(myveh.vehicle,23) == false then
					b.sprite = "garage"
				else
					b.sprite = nil
				end
			elseif b.name == "Pneus customizado" then
				if GetVehicleModVariation(myveh.vehicle,23) then
					b.sprite = "garage"
				else
					b.sprite = nil
				end
			elseif b.name == "Pneu a prova de bala" then
...
RegisterNetEvent('lscustom:sayPayment')
AddEventHandler('lscustom:sayPayment', function(tbl)
	waitingPayment = tbl
end)
...
function CanPurchase(price, canpurchase)
	if canpurchase then
		TriggerServerEvent("lscustom:doPayment",price)
			waitingPayment = 1
			while waitingPayment == 1 do
				Wait(200)
			end
			
			if waitingPayment == 2 then
