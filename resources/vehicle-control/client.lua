...
AddEventHandler('trunk',function() 
	local player = GetPlayerPed(-1)
			...
			if distanceToVeh <= interactionDistance then
				...
				ShowNotification("~r~Трябва да сте близо до автомобила си, за да направите това.")
			end
end)
-- R E A R  D O O R S --
	...
	local distanceToVeh = GetDistanceBetweenCoords(GetEntityCoords(player), GetEntityCoords(vehicle), 1)
		if DoesEntityExist(vehicle) then
			if distanceToVeh <= lockDistance then
				...
				ShowNotification("Заключили сте ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))) .. "~w~.")
				TriggerEvent('lockLights')
				else
				SetVehicleDoorsLocked(vehicle,1)
				ShowNotification("Вие отключихте вашата ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))) .. "~w~.")
				TriggerEvent('lockLights')
				end
			else
				ShowNotification("~r~Трябва да сте близо до автомобила си, за да направите това.")
			...
			ShowNotification("~r~Нямате запазен автомобил.")
		end
	end)
...
AddEventHandler('save',function() 
	...
	if (IsPedSittingInAnyVehicle(player)) then 
		if  saved == true then
			...
			ShowNotification("Запазен автомобил ~r~премахнат~w~.")
			saved = false
		else
			RemoveBlip(targetBlip)
			...
			ShowNotification("Това ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))) .. "~w~ е вашата нова~g~ кола ~w~vehicle.")
			saved = true
		end
	end
end)
