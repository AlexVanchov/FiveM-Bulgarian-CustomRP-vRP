...
local trasureReward = math.random(100000,350000)
...
AddEventHandler('autoStopTreasureHunt', function()
	treasureOn = false
	TriggerClientEvent('chatMessage', -1, "^8[TREASURE-HUNT] ^Никой не намери съкровището!")
...
function startTreasureHunt()
	if(treasureOn == false)then
		...
		TriggerClientEvent('chatMessage', -1, "^8[TREASURE-HUNT] ^2Търсенето на съкровището започна. Отиди на правилното място ^8(Red Crown) ^2и вземи парите!")
		...
		local function treasure_enter(source,area)
			...
			vRPclient.notify(source,{"~r~[TRASURE-HUNT] ~g~Ти получи ~r~$"..treasureReward.." ~g~от съкровището!"})
			vRP.giveMoney({user_id,treasureReward})
			vRP.givediamonds({user_id,treasureDiamonds})
			TriggerClientEvent('chatMessage', -1, "^8[TREASURE-HUNT] ^2"..GetPlayerName(source).." намери съкровището и получи ^5$"..treasureReward)
			treasureOn = false
		end
