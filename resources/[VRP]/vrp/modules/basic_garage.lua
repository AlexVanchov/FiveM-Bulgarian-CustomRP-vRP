...
veh_actions[lang.vehicle.sellTP.title()] = {function(playerID,player,vtype,name)
	if playerID ~= nil then
		vRPclient.getNearestPlayers(player,{15},function(nplayers)
			...
			if usrList ~= "" then
				vRP.prompt(player,"Играчи наблизо: " .. usrList .. "","",function(player,user_id) 
					user_id = user_id
					if user_id ~= nil and user_id ~= "" then 
						local target = vRP.getUserSource(tonumber(user_id))
						if target ~= nil then
							vRP.prompt(player,"Цена $: ","",function(player,amount)
								if (tonumber(amount) and tonumber(amount) >= 0) then
									MySQL.query("vRP/get_vehicle", {user_id = user_id, vehicle = name}, function(pvehicle, affected)
										if #pvehicle > 0 then
											vRPclient.notify(player,{"~r~Играчът вече има този тип превозно средство."})
										else
											local tmpdata = vRP.getUserTmpTable(playerID)
											if tmpdata.rent_vehicles[name] == true then
												vRPclient.notify(player,{"~r~Не можете да продавате нает автомобил!"})
												return
											else
												vRP.request(target,GetPlayerName(player).." Иска да продава: " ..name.. " Цена: $"..amount, 10, function(target,ok)
													if ok then
														...
														if (tonumber(money) >= tonumber(amount)) then
															...
															vRPclient.notify(player,{"~g~Вие успешно продадохте автомобила ".. GetPlayerName(target).." за $"..amount.."!"})
															vRPclient.notify(target,{"~g~"..GetPlayerName(player).." Успешно ви продаде колата $"..amount.."!"})
														else
															vRPclient.notify(player,{"~r~".. GetPlayerName(target).." Няма достатъчно пари!"})
															vRPclient.notify(target,{"~r~Нямате достатъчно пари!"})
														end
													else
														vRPclient.notify(player,{"~r~"..GetPlayerName(target).." Отказа да купи автомобила."})
														vRPclient.notify(target,{"~r~Вие отказахте да купите "..GetPlayerName(player).."колата."})
													end
												end)
											end
											vRP.closeMenu(player)
									...
									vRPclient.notify(player,{"~r~Цената на автомобила трябва да бъде с цифри."})
								end
							end)
						else
							vRPclient.notify(player,{"~r~Това ID е невалидно."})
						end
					else
						vRPclient.notify(player,{"~r~Няма играч с това ID."})
					end
				end)
			else
				vRPclient.notify(player,{"~r~Няма играчи наблизо."})
			end
		end)
	end
